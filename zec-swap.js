...
async function buildSwapTx(rpc, opts) {
  ...
  // IMPORTANT: outputs are built as an ARRAY of single-key objects, not one
  // merged {address: amount} object. zcashd's createrawtransaction accepts
  // either form, but the merged-object form silently collapses multiple
  // outputs to the SAME address into one (later keys overwrite earlier ones
  // in a JS object) — which is exactly the case here, since a buyer's token
  // recipient address and their own change address are very often identical.
  // The array form preserves each output independently even when addresses
  // repeat. Confirmed via a real dry run that the object form drops a real
  // output silently with no error — do not revert to the object form.
  const outputs = [
  ...
  if (buyerChangeAddress && buyerChangeValueZec && buyerChangeValueZec > 0) {
    outputs.push({ [buyerChangeAddress]: zec(buyerChangeValueZec) });
  }
